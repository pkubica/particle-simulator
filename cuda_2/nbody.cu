...
__global__ void particles_simulate(t_particles p_in, t_particles p_out, int N, float dt)
{
	...
	float *p_x = array;
    float *p_y = &array[block_size];
	float *p_z = &array[block_size*2];
	float *p_w = &array[block_size*3];
	...
	{
		ctmp = N - k;
		cond = ctmp < block_size ? ctmp : block_size;
	
		ptr = ti + k;

		if(ptr < N)
		{
			p_x[ti] =  p_in.pos[ptr].x;
			p_y[ti] =  p_in.pos[ptr].y;
			p_z[ti] =  p_in.pos[ptr].z;
			p_w[ti] =  p_in.pos[ptr].weight * G * dt;
		}
